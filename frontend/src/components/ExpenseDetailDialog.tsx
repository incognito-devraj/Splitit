      ...
      {open && expense && (
        <motion.div className="fixed inset-0 z-[70]" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          ...
          <motion.div
            ...
            className="absolute inset-x-0 bottom-0 max-h-[88vh] overflow-hidden rounded-t-[28px] bg-card border-t border-border shadow-2xl"
          >
            <div className="flex items-center justify-between px-5 pt-4">
              ...
              <div>
                ...
                <div className="mt-1 text-lg font-semibold">{expense.title || category?.label}</div>
              </div>
              <button onClick={onClose} className="size-9 rounded-full bg-muted grid place-items-center" aria-label="Close">
                <X className="size-4" />
              </button>
            </div>

            <div className="px-5 pb-5 pt-4 overflow-y-auto max-h-[calc(88vh-4rem)] space-y-4">
              <div className="rounded-3xl gradient-balance p-5 text-white shadow-[var(--shadow-card)]">
                <div className="flex items-center gap-2 text-white/80 text-sm">
                  <span className="text-xl">{category?.emoji}</span>
                  <span>{category?.label}</span>
                </div>
                <div className="mt-2 text-4xl font-semibold tabular">₹{expense.amount.toLocaleString("en-IN")}</div>
                <div className="mt-2 text-sm text-white/80">Paid by {expense.paidBy.name}</div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="rounded-2xl bg-muted/60 p-4">
                  ...
                  <div className="mt-1 text-lg font-semibold">
                    {expense.sharedWith.length + expense.guestParticipants.length}
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-2xl border border-border p-4">
                ...
                <div className="mt-3 space-y-2">
                  {expense.sharedWith.map((person) => (
                    <div key={person._id} className="flex items-center justify-between gap-3 rounded-xl bg-muted/50 px-3 py-2">
                  ...
                  {expense.guestParticipants.map((guest) => (
                    <div key={guest._id} className="flex items-center justify-between gap-3 rounded-xl bg-muted/50 px-3 py-2">
                      <div className="min-w-0">
