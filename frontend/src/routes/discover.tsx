...
function DiscoverPage() {
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: () => groupApi.discover({ search: debouncedSearch, limit: 30 }).then((response) => response.data.data),
  });
  ...
  const activeGroupName = user?.groupId ? "Your active group" : "No active group";
    ...
    <AppShell>
      <div className="px-4 sm:px-6 pt-6">
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Browse public groups and request to join. {activeGroupName}.
        </p>
      </div>
              ...
              {requestingId === group._id && (
                ...
                >
                  <div className="grid grid-cols-2 gap-2">
                    {(["permanent", "occasional"] as const).map((type) => (
                      <button
                        key={type}
                        onClick={() => setMemberType(type)}
                        className={`p-2.5 rounded-xl border text-left text-xs transition-all ${
                          memberType === type ? "border-primary bg-primary/10" : "border-border bg-muted"
                        }`}
                      >
                        <div className="font-semibold">{type === "permanent" ? "👤 Regular" : "⏱ Occasional"}</div>
                        <div className="text-muted-foreground mt-0.5">
                          {type === "permanent" ? "Always split expenses" : "Split when present"}
                        </div>
                      </button>
                    ))}
                  </div>
